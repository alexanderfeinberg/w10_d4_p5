...
-- Step1
  SELECT bands.name, albums.title FROM albums JOIN bands on (albums.band_id = bands.id);


-- Step2
  SELECT bands.name, albums.title FROM albums JOIN bands on (albums.band_id = bands.id) WHERE albums.num_sold <20000;
